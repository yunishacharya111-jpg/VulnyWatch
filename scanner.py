...
HEADERS = {"User-Agent": "Mozilla/5.0 (VulnWatch/1.0)"}
TIMEOUT = 8
...
def run_scan(url):
    ...
    if found_paths:
        results.append(make("Sensitive Paths Exposed", "FAIL", "HIGH",
            ...
            "A01", "Restrict access using authentication and server rules."))
    else:
        results.append(make("Sensitive Paths Exposed", "PASS", "PASS",
            "No sensitive paths found.", "A01", ""))
    ...
    if url.startswith('https://'):
        ...
        results.append(make("HTTPS Enforced", "FAIL", "HIGH",
            ...
            "A02", "Enable HTTPS and redirect all HTTP traffic."))
    ...
    try:
        ...
        if xss_payload in r.text:
            results.append(make("XSS (Reflected)", "FAIL", "HIGH",
                ...
                "A03", "Sanitize and encode all user input before outputting to HTML."))
        else:
    ...
    if sql_found:
        results.append(make("SQL Injection", "FAIL", "CRITICAL",
            "SQL error messages detected.", "A03", "Use parameterised queries only."))
    else:
        results.append(make("SQL Injection", "PASS", "PASS", "No SQL injection found.", "A03", ""))

    # A05 - Security Misconfiguration
    sec_headers = {
        "Content-Security-Policy":   ("CRITICAL", "Add Content-Security-Policy header."),
        "X-Frame-Options":           ("LOW",      "Add X-Frame-Options: DENY."),
        ...
        "Referrer-Policy":           ("INFO",     "Add Referrer-Policy header."),
        "Permissions-Policy":        ("INFO",     "Add Permissions-Policy header."),
    }
    ...
    if server:
        results.append(make("Server Info Disclosure", "WARN", "LOW",
            ...
            "A05", "Remove or obscure the Server header."))
    else:
        results.append(make("Server Info Disclosure", "PASS", "PASS", "Server header not exposed.", "A05", ""))

    # A06 - Outdated Components
    powered = headers.get('X-Powered-By', '')
    if powered:
        results.append(make("Technology Disclosure", "WARN", "LOW",
            f"X-Powered-By: {powered}", "A06", "Remove X-Powered-By header."))
    else:
        results.append(make("Technology Disclosure", "PASS", "PASS", "X-Powered-By not exposed.", "A06", ""))

    # A07 - Cookie Security
    raw_cookies = resp.headers.get('Set-Cookie', '')
    if raw_cookies:
        issues = []
        if 'secure'   not in raw_cookies.lower(): issues.append("missing Secure flag")
        if 'httponly' not in raw_cookies.lower(): issues.append("missing HttpOnly flag")
        if 'samesite' not in raw_cookies.lower(): issues.append("missing SameSite attribute")
        if issues:
            results.append(make("Cookie Security", "FAIL", "MEDIUM",
                f"Issues: {', '.join(issues)}", "A07",
                "Set Secure, HttpOnly, and SameSite=Strict on all cookies."))
        else:
            results.append(make("Cookie Security", "PASS", "PASS", "Cookies have proper flags.", "A07", ""))
    else:
    ...
    try:
        ...
        exposed = [p for p in ['/admin', '/backup', '/config', '/.env', '/private'] if p in r.text]
        if exposed:
            results.append(make("robots.txt Disclosure", "WARN", "LOW",
                f"Sensitive paths exposed: {', '.join(exposed)}", "A09",
                "Remove sensitive paths from robots.txt."))
        else:
            results.append(make("robots.txt Disclosure", "PASS", "PASS",
                "No sensitive paths in robots.txt.", "A09", ""))
    except:
        results.append(make("robots.txt Disclosure", "SKIP", "INFO", "robots.txt not found.", "A09", ""))
    ...
    if open_risky:
        results.append(make("Open Risky Ports", "FAIL", "HIGH",
            f"Open: {', '.join(open_risky)}", "A05",
            "Close unnecessary ports using a firewall."))
    else:
        results.append(make("Open Risky Ports", "PASS", "PASS", "No risky ports open.", "A05", ""))
...
def make(check_name, status, severity, detail, owasp, fix):
    return {"check_name": check_name, "status": status, "severity": severity,
            "detail": detail, "owasp": owasp, "fix": fix}


def calculate_score(results):
    weights = {"CRITICAL": 0.0, "HIGH": 0.3, "MEDIUM": 0.6,
